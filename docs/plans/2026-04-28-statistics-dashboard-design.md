...
为当前账单归档 Web App 增加 M1 统计能力：用户上传一个 Daily/Bar 月度 Excel 和一个 Office 月度 Excel 后，系统生成收入、支出、盈利、每日趋势和支出构成下钻统计图。
...
- 校验 Excel 文件和必要字段。
- 从 Daily/Bar workbook 聚合收入、日常支出、每日趋势和 Bar Ausgabe 每日汇总下钻。
- 从 Office workbook 聚合总支出、按 `Type` 下钻和明细。
- 合并 Daily/Bar 支出与 Office type 支出，形成统一的支出构成数据。
- 返回稳定 JSON contract。
- 统计结束后，用 `shutil.rmtree` 清理上传的临时文件目录（在 `finally` 块中执行，无论解析是否成功）。统计结果已在响应中返回，临时文件不需要保留。

前端负责：

- 提供两个 Excel 上传控件。
- 调用统计 preview API。
- 展示 KPI、收入支出桥图、支出构成饼图、每日趋势、下钻明细和 warning。
...
`expense_breakdown` 用于前端渲染统一支出构成饼图。`daily_expense_rows` 用于点击 `Bar Ausgabe` 后展示 Daily/Bar 每日支出汇总。`office_rows` 用于点击某个 Office `Type` 后展示明细。M1 不需要再做 Office 明细二次分页，Excel 行数可控。
...
- 生成 `daily_expense_rows`：
  - 只包含 `daily_expense_brutto > 0` 的日期行。
  - 同一天有多行时按日期汇总。
  - 用于 `Bar Ausgabe` 的下钻表。
- 空金额按 `0` 处理。
...
- 使用 active sheet。
- 必须存在 `Type`、`Brutto`。
- 可选列：`Datum`（映射到 `date`，空或缺失时为 `null`）、`Rechnung Name`（映射到 `name`，空或缺失时为 `null`）。
- 每行：
...
Unified expense breakdown:

- `Bar Ausgabe` 作为一个独立支出分类，金额为全月 Daily/Bar `Ausgabe <N> Brutto` 合计。
- Office 每个 `Type` 作为一个独立支出分类，金额为该 type 的 `Brutto` 合计。
- `expense_breakdown` 总额 = `Bar Ausgabe + Office 各 type Brutto`。
- `share = category_brutto / expense_breakdown_total`，总额为 0 时 share 为 0。
- 按 `brutto` 降序排序；`Bar Ausgabe` 不特殊置顶，按金额自然排序。
...
4. 第一行图表：
   - 左侧 Profit bridge：纯 SVG 四柱对比图，依次展示收入（绿/蓝）、Daily 支出（红）、Office 支出（红）和净盈利（绿或红取决于正负），柱高按绝对值等比缩放。M1 不做真正的瀑布图，四柱并排对比即可。
   - 右侧 Expense breakdown：支出构成饼图，`Bar Ausgabe` 算一类，Office 每个 `Type` 各算一类。
5. Expense drilldown：点击饼图扇区或分类列表后展示对应明细：
   - `Bar Ausgabe`：展示 Daily/Bar 每日支出汇总，字段为日期和 Brutto。
   - Office type：展示该 type 下的 Office 明细，字段为日期、名称和 Brutto。
6. 第二行图表：Daily trend，按日展示收入、Daily 支出和日净额。
7. Warnings panel：展示后端 warning。

M1 不新增图表依赖，优先用轻量 SVG 组件实现柱状、折线、桥图和饼图。当前图表数量少，避免引入 Recharts 等依赖带来的打包、样式和测试成本。若后续统计页面明显扩展，再评估图表库。
