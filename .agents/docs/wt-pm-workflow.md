...
| 阶段 | 操作者 | 说明 |
|------|--------|------|
| 1. Task Creation | PM | 在 `plans/todo_current.md` 创建 `UNPLANNED` 条目 |
| 2. Worktree Setup | Executor | `git worktree add -b feat/<id>-<slug> <path> main` |
| 3. Planning | Executor | 创建 plan 文件，task 状态 → `PLANNED` |
| 4. Implementation | Executor | 编码、单元测试、迭代 |
| 5. Sync Main | Executor | `git merge main`，解决冲突 |
| 6. Regression Gate | Executor | 运行完整回归测试套件 |
| 7. Merge & Cleanup | Executor | 合并到 main，删除 worktree |
